...
import getMoves from "../../basic/getMoves";

import isChecked from "../../basic/isChecked";
...
export function setNextMovableTiles() {
  return (dispatch, getState) => {
    ...
    const { turn, checkedByPieces, snapshot, snapshotMove, kingMoved } =
      present || {};
    const hasKingMoved = turn === "w" ? kingMoved[0] : kingMoved[1];
    let { moves: newMoves, specialMoves } = getMoves(
      snapshot,
      turn,
      moves,
      snapshotMove,
...
export function setNextSnapshot(newMove, promotionPiece = "") {
  return (dispatch, getState) => {
    ...
    if (
      !selected ||
      !selected.charAt(0) === turn ||
      selectedMoves.indexOf(newMove) === -1
    )
      return;
    ...
    specialMoves.forEach((moveArray) => {
      const firstMove = moveArray[0],
        secondMove = moveArray[1];
      if (firstMove.substring(4) !== newMove) return;

      if (firstMove.charAt(1) === "K") {
        // Check if its castling move
    ...
    // Check if checked
    const { firstCheckingPiece, secondCheckingPiece } = isChecked(
      newSnapshot,
      turn
    );
    if (firstCheckingPiece) {
      ...
      // Clear check from previous turn if checked
      dispatch(setCheckedByPieces(0));
    }

    if (selected.charAt(1) === "K") {
      // Check if its a king move
