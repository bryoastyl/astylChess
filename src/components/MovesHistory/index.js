  ...
  return (
    <div className={css.container}>
      ...
      {/* <div className={css.arrows}>
        <button className={css.arrow} onClick={props.undoSnapshot}>
          {" "}
          <ArrowLeft />{" "}
        </button>
        <button className={css.arrow} onClick={props.redoSnapshot}>
          {" "}
          <ArrowRight />{" "}
        </button>
      </div> */}
...
const mapStateToProps = (state) => {
  ...
  let {
    snapshotMove: currSnapshotMove,
    moves,
    checkedByPieces: currCheckedByPieces,
    captured: currCaptured,
  } = present;
  ...
  past.forEach((pastState, index) => {
    ...
    if (snapshotMove.charAt(1) === "P") {
      ...
      if (captured)
        snapshotMove = snapshotMove.slice(0, 2) + "x" + snapshotMove.slice(2);
    } else if (snapshotMove !== "O-O" && snapshotMove !== "O-O-O") {
      snapshotMove = snapshotMove.slice(1, 2) + snapshotMove.slice(4, 6);
      if (captured)
        snapshotMove =
          snapshotMove.slice(0, 1) + "x" + snapshotMove.slice(1, 3);
    }
  ...
  if (currSnapshotMove.charAt(1) === "P") {
    ...
    if (currCaptured)
      currSnapshotMove =
        currSnapshotMove.slice(0, 2) + "x" + currSnapshotMove.slice(2);
  } else if (currSnapshotMove !== "O-O" && currSnapshotMove !== "O-O-O") {
    currSnapshotMove =
      currSnapshotMove.slice(1, 2) + currSnapshotMove.slice(4, 6);
    if (currCaptured)
      currSnapshotMove =
        currSnapshotMove.slice(0, 1) + "x" + currSnapshotMove.slice(1, 3);
  }
  ...
  future.forEach((futureState, index) => {
    ...
    futureMoveList.push({
      ...
      snapshotMove,
      past: false,
      key: pastMoveList.length + 1 + index,
    });
  });
  ...
  moveList = [
    ...pastMoveList,
    { index: -1, snapshotMove: currSnapshotMove, key: pastMoveList.length },
    ...futureMoveList,
  ];
